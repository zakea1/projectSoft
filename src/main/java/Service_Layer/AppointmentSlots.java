...
public class AppointmentSlots {
    ...
    public String bookAppointment(int index, String email, AppointmentType type, int participantsCount) {
        ...
        if (!isValidIndex(schedule, index)) return INVALID_INDEX_MSG;
        TimeSlot slot = getSlot(schedule, index);
        if (slot.getStart().isBefore(LocalDateTime.now())) return "Cannot book past appointments.";
        if (slot.isBooked()) return ALREADY_BOOKED_MSG;
        if (findUserBooking(schedule, email) != null) return "User already has a booking.";
        if (!AppointmentRules.validateParticipants(type, participantsCount)) return "Invalid participants for " + type;
    ...
    public String cancelAppointment(String email) {
        ...
        if (slot == null) return NO_BOOKING_MSG;
        slot.cancel();
        saveSchedule(schedule);
    ...
    public String modifyAppointment(String email, int newIndex, AppointmentType newType) {
        Schedule schedule = getSchedule();
        if (!isValidIndex(schedule, newIndex)) return INVALID_INDEX_MSG;
        TimeSlot oldSlot = findUserBooking(schedule, email);
        if (oldSlot == null) return NO_BOOKING_MSG;
        TimeSlot newSlot = getSlot(schedule, newIndex);
        if (newSlot.isBooked()) return NEW_SLOT_ALREADY_BOOKED_MSG;
        if (!AppointmentRules.validateParticipants(newType, oldSlot.getParticipants())) return "Invalid participants for " + newType;
    ...
    public String modifyAppointmentByAdmin(int oldIndex, int newIndex) {
        Schedule schedule = getSchedule();
        if (!isValidIndex(schedule, oldIndex) || !isValidIndex(schedule, newIndex)) return INVALID_INDEX_MSG;
        TimeSlot oldSlot = getSlot(schedule, oldIndex);
        TimeSlot newSlot = getSlot(schedule, newIndex);
        if (!oldSlot.isBooked()) return OLD_SLOT_NOT_BOOKED_MSG;
        if (newSlot.isBooked()) return NEW_SLOT_ALREADY_BOOKED_MSG;
